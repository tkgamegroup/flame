	...
	if (argc >= 2)
	{
		...
		if (what == "component_source")
		{
			if (argc > 2)
			{
				...
				private_header_file << "#include \"" << class_name << ".h\"\n\n";
				public_header_file << "namespace flame\n{\n";
				public_header_file << "\tstruct " << class_name << "Private : " << class_name << "\n\t{\n";
				public_header_file << "\t};\n}\n";
				private_header_file.close();

				std::ofstream source_file(name + ".cpp");
				source_file << "#include \"" << class_name << "_private.h\"\n\n";
				public_header_file << "namespace flame\n{\n";
				public_header_file << "\t" << class_name << "* " << class_name << "::create(void* parms)\n\t{\n";
				public_header_file << "\t\treturn new " << class_name << "Private();\n";
				public_header_file << "\t}\n}\n";
				source_file.close();
			}
		}
