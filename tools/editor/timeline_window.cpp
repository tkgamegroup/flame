...
void TimelineView::on_draw()
{
	...
	ImGui::SmallButton(graphics::font_icon_str("backward-fast"_h).c_str());
	ImGui::SameLine();
	if (ImGui::SmallButton(graphics::font_icon_str("backward-step"_h).c_str()))
		app.set_timeline_current_frame((int)app.timeline_current_frame - 1);
	ImGui::SameLine();
	if (ImGui::SmallButton(graphics::font_icon_str("play"_h).c_str()))
		app.timeline_toggle_playing();
	ImGui::SameLine();
	if (ImGui::SmallButton(graphics::font_icon_str("forward-step"_h).c_str()))
		app.set_timeline_current_frame((int)app.timeline_current_frame + 1);
	ImGui::SameLine();
	ImGui::SmallButton(graphics::font_icon_str("forward-fast"_h).c_str());
	ImGui::PopStyleVar();
	ImGui::SameLine();
	...
	{
		if (ImGui::Button(graphics::font_icon_str("circle-dot"_h).c_str()))
	...
	ImGui::PopItemWidth();
	ImGui::SameLine();
	if (ImGui::SmallButton((graphics::font_icon_str("xmark"_h) + "##asset").c_str()))
		app.close_timeline();
	ImGui::SameLine();
	if (ImGui::SmallButton(graphics::font_icon_str("floppy-disk"_h).c_str()))
	{
		if (app.opened_timeline)
	...
	ImGui::PopItemWidth();
	ImGui::SameLine();
	if (ImGui::SmallButton((graphics::font_icon_str("xmark"_h) + "##host").c_str()))
	...
	ImGui::InvisibleButton("##timeline", ImVec2(ImGui::GetContentRegionAvailWidth(), bar_height));
	{
		...
		}
		{
			...
			dl->AddText(ImVec2(x - 4.5f, timeline_p1.y - bar_height - 4.f), ImColor(200, 200, 200), graphics::font_icon_str("chevron-down"_h).c_str());
