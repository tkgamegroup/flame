...
{
	auto eos = editing_objects_list.top().get();
	switch (eos->type())
	{
	case 0:
	{
		auto eas = (EditingAssets*)eos;
		add_history(new AssetModifyHistory(eas->paths, eas->asset_type, attr_hash, before_editing_values, new_value));
	}
		break;
	case 1:
	{
		auto ees = (EditingEntities*)eos;
		add_history(new EntityModifyHistory(ees->ids, 0, attr_hash, before_editing_values, new_value));
	}
		break;
	case 2:
	{
		auto ecs = (EditingComponents*)eos;
		add_history(new EntityModifyHistory(ecs->ids, ecs->comp_type, attr_hash, before_editing_values, new_value));
	}
		break;
	}
}

bool show_variable(TypeInfo* type, const std::string& name, uint name_hash, int offset, const FunctionInfo* getter, const FunctionInfo* setter, const std::vector<void*>& objs, const void* id)
{
	...
	case TagD:
	{
		...
		break;
	case TagVD:
		if (objs.size() == 1 && ImGui::TreeNode(display_name.c_str()))
		{
			assert(!getter);
			auto obj = objs[0];
...
void View_Inspector::on_draw()
{
	...
	case Selection::tEntity:
	{
		...
		if (auto e = entities.size() == 1 ? entities[0] : nullptr; e && e->prefab_instance)
		{
		...
		//						{
		//							editing_objects.emplace(new EditingAsset(name, th<graphics::Material>()));
