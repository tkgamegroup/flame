function terrain_spawn(extent, height_tex, e_dst, poses, prefab, y_off)
    ...
    for i=0, num-1, 1 do
        flame_set(uvs, 8 * i, e_type_data, e_floating_type, 2, 1, poses[i + 1] * one_over_ext)
    end
    ...
    for i=0, num-1, 1 do
        ...
        node.set_pos(vec3(
            poses[i + 1].x, 
            flame_get(heights, 16 * i, e_type_data, e_floating_type, 4, 1).x * extent.y + y_off, 
            poses[i + 1].y
        ))
        e_dst.add_child(e)
    end 
	    ...
	    for x = 0, cx - 1, 1 do
            ...
            if math.random() < probability and dot_3(normal, vec3(0, 1, 0)) > 0.9 then
                ...
                node.set_pos(vec3(
                    ...
                    flame_get(heights, 16 * i, e_type_data, e_floating_type, 4, 1).x * extent.y, 
