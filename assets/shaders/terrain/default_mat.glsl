...
float tiling = float(material.i[0]);

vec4 color = vec4(0);
#if LAYERS == 1
	vec4 weights = texture(terrain_splash_maps[i_id], i_uv);
	if (weights[0] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[0], tiling) * weights[0];
#elif LAYERS == 2
	vec4 weights = texture(terrain_splash_maps[i_id], i_uv);
	if (weights[0] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[0], tiling) * weights[0];
	if (weights[1] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[1], tiling) * weights[1];
#elif LAYERS == 3
	vec4 weights = texture(terrain_splash_maps[i_id], i_uv);
	if (weights[0] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[0], tiling) * weights[0];
	if (weights[1] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[1], tiling) * weights[1];
	if (weights[2] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[2], tiling) * weights[2];
#elif LAYERS == 4
	vec4 weights = texture(terrain_splash_maps[i_id], i_uv);
	if (weights[0] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[0], tiling) * weights[0];
	...
	if (weights[2] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[2], tiling) * weights[2];
	if (weights[3] > 0.0)
		color.rgb += textureTriPlanar(material.map_indices[3], tiling) * weights[3];
#endif
