...
int main(int argc, char **args)
{
...

	{
		...
		e_page->add_child(e_btn_run);
		{
			...
			}, new_mail_p(e_btn_run->find_component(cH("Text"))));
		}
	...
	auto e_console = open_console([](void* c, const std::wstring& cmd, cConsole* console) {
	...
	}, new_mail_p(nullptr), Vec2f(850.f, 420.f), app.font_atlas_pixel);

	app.root->add_child(e_console);
	{
		auto c_console = (cConsole*)e_console->find_component(cH("Console"));
		c_console->print(app.filename + L":");
	}
