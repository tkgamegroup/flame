...
namespace flame
{
	...
	struct BP_Vec
	{
		...
		{
			auto u = add_udt(level, "Vec~" + template_parameters, sizeof(BP_Vec), L"../renderpath/canvas/bp.dll");
	...
	struct BP_Array
	{
		...
		static void add_udt_info(int level, const std::string& template_parameters, void* module)
		{
			...
			auto u = add_udt(level, "Array~" + sp[0] + "~" + sp[1], sizeof(BP_Array), L"../renderpath/canvas/bp.dll");
			...
			for (auto i = 0; i < N; i++)
				u->add_variable(tag, sp[1], std::to_string(i + 1), "i", sizeof(AttributeV<T>) * i, sizeof(AttributeV<T>));
			u->add_variable(TypeTagAttributeV, "std::vector~" + sp[1] + (tag == TypeTagAttributeP ? "*" : ""), "v", "o", offsetof(BP_Array, out), sizeof(BP_Array::out));
