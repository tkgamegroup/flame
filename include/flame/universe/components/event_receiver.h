...
namespace flame
{
	...
	struct cEventReceiver : Component
	{
		...
		FLAME_UNIVERSE_EXPORTS void* add_mouse_listener(void (*listener)(void* c, KeyState action, MouseKey key, const Vec2i& pos), const Mail<>& capture);
		FLAME_UNIVERSE_EXPORTS void* add_drag_and_drop_listener(void (*listener)(void* c, DragAndDrop action, cEventReceiver* er, const Vec2i& pos), const Mail<>& capture);
		...
		FLAME_UNIVERSE_EXPORTS void on_mouse(KeyState action, MouseKey key, const Vec2i& value);
