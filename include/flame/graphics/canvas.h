...
namespace flame
{
	namespace graphics
	{
		...
		struct Canvas
		{
			...
			FLAME_GRAPHICS_EXPORTS Vec2f add_text(FontAtlas* f, const Vec2f& pos, const Vec4c& col, const std::wstring& text, float scale/* for sdf */, const Vec4f& scissor);
			FLAME_GRAPHICS_EXPORTS void add_text_right_align(FontAtlas* f, const Vec2f& pos, const Vec4c& col, const std::wstring& text, float scale/* for sdf */, const Vec4f& scissor);
