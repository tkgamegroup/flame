...
namespace flame
{
		...
		if (view_layout && target_element)
		{
			if (type == ScrollerHorizontal)
			{
				...
				if (target_size > view_element->width)
					thumb_element->set_width(view_element->width / target_size * track_element->width);
				else
	...
	void cScrollerPrivate::on_gain_view()
	{
		((EntityPrivate*)view->entity)->add_local_data_changed_listener([](Capture& c, Component* t, uint64 h) {
			auto thiz = c.thiz<cScrollerPrivate>();
