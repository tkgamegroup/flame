...
//
//			FLAME_GRAPHICS_EXPORTS static RenderPreferences* create(DevicePtr device, bool hdr = true);
//		};
//
...
//
//			virtual ImageViewPtr get_output(uint idx) const = 0;
//			virtual void set_output(uint views_count, ImageViewPtr const* views) = 0;
//
...
//
//			virtual ImageViewPtr get_texture_resource(uint slot) = 0;
//			virtual int find_texture_resource(const char* name) = 0;
//			virtual uint set_texture_resource(int slot /* -1 to find an empty slot */, ImageViewPtr iv, Sampler* sp, const char* name = nullptr) = 0;
//
...
//			virtual void set_sky(ImageViewPtr box, ImageViewPtr irr, ImageViewPtr rad, ImageViewPtr lut) = 0;
//
