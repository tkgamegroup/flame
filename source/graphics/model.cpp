		...
		namespace model
		{
			...
			void ModelPrivate::save(const std::filesystem::path& _filename)
			{
				...
				if (extension == L".fmodb")
				{
					...
					std::function<void(NodePrivate*)> traverse;
					traverse = [&](NodePrivate* n) {
						write_s(file, n->name);
						write_t(file, n->pos);
						write_t(file, n->qut);
						write_t(file, n->scl);
				...
				else if (extension == L".fmod")
				{
					...
					for (auto& a : animations)
					{
						...
						for (auto& c : a->channels)
						{
							...
							n_channel.append_child("position_keys").append_attribute("data").set_value(base64::encode((char*)c->position_keys.data(), c->position_keys.size() * sizeof(animation::PositionKey)).c_str());
							n_channel.append_child("rotation_keys").append_attribute("data").set_value(base64::encode((char*)c->rotation_keys.data(), c->rotation_keys.size() * sizeof(animation::RotationKey)).c_str());
						}
					}
			...
			ModelPrivate* ModelPrivate::get(const std::filesystem::path& filename)
			{
					...
					for (auto n_animation : doc_root.child("animations"))
					{
						...
						for (auto n_channel : n_animation.child("channels"))
						{
							...
							c->node_name = n_channel.attribute("node_name").value();
							{
								...
								c->position_keys.resize(res.size() / sizeof(animation::PositionKey));
							...
							}
							{
								...
								c->rotation_keys.resize(res.size() / sizeof(animation::RotationKey));
				...
				else
				{
					...
					for (auto i = 0; i < scene->mNumAnimations; i++)
					{
						...
						auto dst = new animation::AnimationPrivate;
						...
						for (auto j = 0; j < src->mNumChannels; j++)
						{
							...
							auto dst_c = new animation::ChannelPrivate;
