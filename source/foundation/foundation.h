...
namespace flame
{
	...
	enum KeyboardKey
	{
		...
		Keyboard_End, Keyboard_Home,
		Keyboard_Left, Keyboard_Up, Keyboard_Right, Keyboard_Down,
		Keyboard_PrtSc,
		Keyboard_Ins,
		Keyboard_Del,
		Keyboard_GraveAccentAndTilde,
	...
	inline std::vector<std::string> format_defines(const std::string& str)
	{
		...
		for (auto view : SUS::split(str, ';'))
		{
			auto s = SUS::get_trimed(view);
			if (!s.empty())
				ret.push_back(s);
		}
	...
	FLAME_FOUNDATION_API void* load_preset_file(const std::filesystem::path& _filename, void* obj = nullptr, UdtInfo** out_ui = nullptr);
